...
class FoundationEstacas(models.Model):
    ...
    Atributos:
        ...
        profundidade (Float): Profundidade da estaca em metros,
        com validação para estar entre 1 e 40 metros.
    ...
    foundation_maquina_registro_id = fields.Many2one('foundation.maquina.registro',
                                                     string="Foundation Maquina registro",
                                                     required=True)
    ...
    def _compute_display_medicao(self):
        ...
        for record in self:
            # Certifique-se de que nome_medicao é uma string e contém apenas números
            if isinstance(record.nome_medicao, str) and record.nome_medicao.isdigit():
                ...
                record.display_medicao = ""  # Um valor padrão ou erro se o nome não for válido
    ...
    def _compute_line_values(self):
        """
            ...
            """
        for record in self:
            record.unit_price = record.sale_order_line_id.price_unit if \
                record.sale_order_line_id else 0
