...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, **kwargs):
        """
        Sobrescrita do método para incluir estacas relacionadas sem medição nas linhas da fatura e criar uma nova medição.
        """
        invoice_vals_list = super(SaleOrder, self)._create_invoices(grouped=grouped, final=final, **kwargs)
        invoices = self.env['account.move'].browse([vals['id'] for vals in invoice_vals_list])
        ...
        for invoice in invoices:
            ...
            if estacas:
                ...
                for estaca in estacas:
                    line_vals = {
                        'move_id': invoice.id,
                        'product_id': estaca.variante_id.id,
                    ...
                    invoice_line = InvoiceLine.create(line_vals)
