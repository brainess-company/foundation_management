        ...
        for invoice in invoices:
            ...
            if estacas:
                ...
                for estaca in estacas:
                    ...
                    _logger.info(f'Stake {estaca.nome_estaca} (ID: {estaca.id}) linked to measurement ID {new_medicao.id}')

                    # Adicionando linha de fatura para cada estaca
                    line_vals = {
                        'move_id': invoice.id,
                        'product_id': estaca.sale_order_line_id.product_id.id,  # Usando o product_id da linha de pedido
                    ...
                    _logger.info(f'Invoice line for stake {estaca.nome_estaca} added to invoice ID {invoice.id}: {new_line.name}')

                # Associando a nova medição com a fatura corretamente
                new_medicao.invoice_id = invoice
                _logger.info(f'Measurement {nome_medicao} (ID: {new_medicao.id}) is now linked with invoice ID {invoice.id}')
